...
const fillDB = async (sequelize, DB_SIZE = 1, DAYS_COUNT = 1) => {
  ...
  const getRandomDate = (base, direction = -1) => {
    return new Date(+base + direction * (60000 * Generator.genInt(0, 0, 60) * Generator.genInt(0, 12, 24) * Generator.genInt(1)) + direction) 
  }
  ...
  for (let i = 0; i < DAYS_COUNT; i += 1) {
    ...
    for (const row of index) {
      ...
      if (count > 0) {
        ...
        const statuses = ['created', 'accepted', 'confirmed', 'shipped'] 
    ...
    for (const brw of breweries) {
      ...
      if (senderId !== receiverId) {
        await DS.Transfer.create({
          ...
          senderId,
          receiverId,
          product_id: products[Generator.genInt(0, 0, products.length - 1)].id
        })
      }
    }
  }
  ...
  let result = await sequelize.query('SELECT order_date FROM orders ORDER BY orders.order_date ASC LIMIT 1', {})
  ...
  for (const dc of dcs) {
    ...
    for(const product of products) {
      await DS.Inventory.create({
        date,
        net: Generator.genInt(0, Generator.genInt(2), Generator.genInt(4)),
        add: 0,
        sales: 0,
